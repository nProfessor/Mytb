...
class CFavorites
{
	...
	{
		$dbFav = CFavorites::GetList(array(),array("URL" => "'%".$url."'"));
		$arFav = $dbFav->Fetch();

		return $arFav["ID"] ? $arFav["ID"] : 0;
	}
...
class CBXFavAdmMenu
{
		...
		if(!empty($this->arItems))
			foreach ($this->arItems as $arItem)
			{
				...
				if(is_array($tmpMenu))
				{
					...
					elseif($this->CheckItemActivity($tmpMenu))
			...
			$aMenu[] = array(
						"text" => GetMessage("fav_main_menu_header"),
						"title" => GetMessage("fav_main_menu_header"),
						"url" => "favorite_list.php?lang=".LANG,
						"dynamic" => false,
						"icon" => "fav_menu_icon",
						"page_icon" => "fav_page_icon"
					);
	...
	private function CheckItemActivity($arMenu)
	{
		...
		if("/bitrix/admin/".$arMenu["url"] == $currentUrl)
			return true;

		if($hostPos=strpos($arMenu["url"], $_SERVER["HTTP_HOST"]))
			$favUrlWithoutHost=substr($arMenu["url"],$hostPos+strlen($_SERVER["HTTP_HOST"]));
		else
			$favUrlWithoutHost = $arMenu["url"];

		$activeSectUrl = $adminMenu->aActiveSections["_active"]["url"];

		if($arMenu["url"] == $activeSectUrl || $favUrlWithoutHost == "/bitrix/admin/".$activeSectUrl)
			return true;
	...
	public function GenerateMenuHTML($id = 0)
	{
		...
		foreach ($menuItems as $arItem)
			$adminMenu->Show($arItem);

		echo '<div id="last-fav-addition-id" style="display:none;">'.$id.'</div>';
