...
?>
			<td class="adm-left-side-wrap">

<script type="text/javascript">
BX.adminMenu.setActiveSection('<?=$openedSection?>');
BX.adminMenu.setOpenedSections('<?=CUtil::JSEscape($adminMenu->GetOpenedSections());?>');
</script>
				<div class="adm-left-side"<?if(intval($aOptMenuPos["width"]) > 0) echo ' style="width:'.intval($aOptMenuPos["width"]).'px"'?> id="bx_menu_panel">
...
<?
	...
	foreach($adminMenu->aGlobalMenu as $menu):
		...
		if ($menu['url']):
?>
						<a href="<?=htmlspecialcharsbx($menu["url"])?>" class="<?=$menuClass?>" onclick="BX.adminMenu.GlobalMenuClick('<?echo $menu["menu_id"]?>'); return false;" onfocus="this.blur();" id="global_menu_<?echo $menu["menu_id"]?>">
...
?>
						<span class="<?=$menuClass?>" onclick="BX.adminMenu.GlobalMenuClick('<?echo $menu["menu_id"]?>'); return false;" id="global_menu_<?echo $menu["menu_id"]?>">
							<div class="adm-main-menu-item-icon"></div>
							<div class="adm-main-menu-item-text"><?echo htmlspecialcharsbx($menu["text"])?></div>
							<div class="adm-main-menu-hover"></div>
						</span>
<?
		...
		$menuScripts .= "BX.adminMenu.registerItem('global_menu_".$menu["menu_id"]."', {ID:'".CUtil::JSEscape($menu['items_id'])."'});";
...
?>
						<div id="global_submenu_<?echo $menu["menu_id"]?>" style="display: <?=$subMenuDisplay?>">
<?
