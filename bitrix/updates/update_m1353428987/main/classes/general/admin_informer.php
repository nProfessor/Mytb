...
<div class="adm-informer" id="admin-informer" style="display: none; top:48px; left:316px;" onclick="return BX.adminInformer.OnInnerClick(event);">
	<div class="adm-informer-header">'.GetMessage("MAIN_AI_NEW_NOTIF").'</div>';
		...
		$div .= '
	<a href="javascript:void(0);" class="adm-informer-footer" id="adm-informer-footer" onclick="BX.adminInformer.ShowAll();" '.( $itemsCount <= $visibleCount ? ' style="display:none;"' : '' ).'>'.GetMessage('MAIN_AI_ALL_NOTIF').'</a>
	...
	public static function InsertMainItems()
	{
		...
		if($USER->IsAdmin() || $USER->CanDoOperation('install_updates'))
		{
			...
			else // update_autocheck == false
			{
				...
				$updAIParams["HTML"] = '<span class="adm-informer-strong-text">'.GetMessage("top_panel_ai_sys_ver").'</span> '.SM_VERSION."<br><br>";
				$updAIParams["HTML"] .= $updateDate ? GetMessage("top_panel_ai_upd_last").' '.$updateDate : GetMessage("top_panel_ai_upd_never");
