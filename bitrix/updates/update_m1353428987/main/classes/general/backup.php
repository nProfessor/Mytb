...
class CBackup
{
		...
		while($i <= (count($ptab) - 1) && haveTime())
		{
			if (strlen($ptab[$i]))
			{
				if($last_row == -1)
				{
					...
					$dump = CBackup::createTable($ptab[$i], $drop);
					fwrite($f, $dump."\n");
					$next = false;
					$ret["num"] = $i;
					$ret["st_row"] = 0;
	...
	function prepare_preg_escape($arSkipMask)
	{
		static $res;
		if (!isset($res))
			foreach($arSkipMask as $a)
				$res[] = CBackup::_preg_escape($a); 
		return $res;
	}
	...
	function skipMask($abs_path)
	{
		...
		if (!$preg_mask_array)
			$preg_mask_array = CBackup::prepare_preg_escape($arSkipMask);

		reset($arSkipMask);
		foreach($arSkipMask as $k => $mask)
		{
	...
	{
		$arc_name = DOCUMENT_ROOT.BX_ROOT."/backup/".date("Y-m-d_H-i");
