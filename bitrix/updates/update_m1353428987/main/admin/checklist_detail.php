...
?>
		...
		<?if($arPoints[$arTestID]["AUTO"] == "Y"):?>
			<div class="checklist-popup-start-test-block checklist-popup-name-test">
				<a onclick="StartPointAutoCheck()" class="adm-btn adm-btn-green adm-btn">
					<span style="color: #ffffff; font-weight: bold"><?=GetMessage("CL_AUTOTEST_START");?></span>
				</a>
...
?>		
	...
	function Move(action)
	{
		...
		ShowWaitWindow();
		BX.ajax.post(
			"/bitrix/admin/checklist_detail.php?TEST_ID="+arStates["POINTS"][current].TEST_ID+"&lang=<?=LANG;?>&bxpublic=Y",
			data,
			function(data)
			{
				ReCalc(current);
