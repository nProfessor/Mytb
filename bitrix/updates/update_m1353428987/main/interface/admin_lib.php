...
class CAdminMenu
{
	...
	{
		static $START_MAGIC_NUMBER = 30, $STEP_MAGIC_NUMBER = 28;
	...
	function Show($aMenu, $level=0)
	{
		...
		$icon = isset($aMenu["icon"]) && $aMenu["icon"] <> ""
			? '<span class="adm-submenu-item-link-icon" id="'.$aMenu["icon"].'"></span>'
//			: ($level < 1 ? '<span class="adm-submenu-item-link-icon" id="default_menu_icon"></span>' : '');
			: '';
		$id = 'menu_item_'.RandString(10);
?>
<div class="adm-sub-submenu-block<?=$level > 0 ? ' adm-submenu-level-'.($level+1) : ''?><?=$bSectionActive && isset($aMenu["items"]) && is_array($aMenu["items"]) && count($aMenu['items']) > 0 ? ' adm-sub-submenu-open' : ''?><?=$aMenu["_active"] ? ' adm-submenu-item-active' : ''?><?=!$bSubmenu ? ' adm-submenu-no-children' : ''?>">
	<div class="adm-submenu-item-name" id="<?=$id?>">
<?
		$onclick = '';
		if ($bSubmenu)
		{
...
?>
		<span class="adm-submenu-item-arrow"<?=$level > 0 ? ' style="width:'.$this->_get_menu_item_width($level).'px;"' : ''?><?=$onclick ? ' onclick="'.$onclick.'"' : ''?>></span>
<?
...
class CAdminFilter
{
	...
	private $arOptFlt = false;

	public function __construct($id, $popup=false)
	{
		global $USER;
		...
		$this->arOptFlt = CUserOptions::GetOption("filter", $this->id, array("rows" => ""));
	...
	private function AddItem($arItem, $bInsertFirst = false)
	{
		...
		if( $this->FindItemByPresetId($arItem["ID"]))
			return false;
	...
	public function AddPreset($arFields)
	{
		...
		$item = array(
			...
			"FILTER_ID" => $this->id,
			"NAME" => $arFields["NAME"],
			"FIELDS" => CAdminFilter::FieldsExcess($arFields["FIELDS"]),
			"EDITABLE" => false
			);
	...
	public function GetList($aSort=array(), $arFilter=Array(), $getCommon=true)
	{
		...
		$strSql = "
			SELECT
				F.ID, F.USER_ID, F.NAME, F.FILTER_ID, F.FIELDS, F.COMMON, F.PRESET, F.LANGUAGE_ID, F.PRESET_ID
			FROM
				b_filters F
			WHERE
			".$strSqlSearch."
	...
	{
		echo '
<div id="adm-filter-tab-wrap-'.$this->id.'" class="adm-filter-wrap" style = "display: none;">
	<table class="adm-filter-main-table">
		...
		if(is_array($this->arItems) && !empty($this->arItems))
		{
			...
			echo '<span id="adm-filter-add-tab-'.$this->id.'" class="adm-filter-tab adm-filter-add-tab" onclick="'.$this->id.'.CreateNewFilter();" title="'.GetMessage("admin_lib_filter_new").'"></span>
				</div>
			</td>
		</tr>
		<tr>
	...
	public function Buttons($aParams=false)
	{
		...
		if($aParams !== false)
		{
			...
			{
				echo '
						<input type="submit" class="adm-btn" id="'.$this->id.'set_filter" name="set_filter" title="'.GetMessage("admin_lib_filter_set_rep_title").$hkInst->GetTitle("set_filter").'" onClick="'.htmlspecialcharsbx($this->id.'.OnSet(\''.CUtil::AddSlashes($aParams["table_id"]).'\', \''.CUtil::AddSlashes($url).'\'); return false;').'" value="'.GetMessage("admin_lib_filter_set_rep").'">
						<input type="submit" class="adm-btn" id="'.$this->id.'del_filter" name="del_filter" title="'.GetMessage("admin_lib_filter_clear_butt_title").$hkInst->GetTitle("del_filter").'" onClick="'.htmlspecialcharsbx($this->id.'.OnClear(\''.CUtil::AddSlashes($aParams["table_id"]).'\', \''.CUtil::AddSlashes($url).'\'); return false;').'" value="'.GetMessage("admin_lib_filter_clear_butt").'">';
			}
			else
				echo '
						<input type="submit" class="adm-btn" id="'.$this->id.'set_filter" name="set_filter" title="'.GetMessage("admin_lib_filter_set_butt").$hkInst->GetTitle("set_filter").'" onClick="'.htmlspecialcharsbx($this->id.'.OnSet(\''.CUtil::AddSlashes($aParams["table_id"]).'\', \''.CUtil::AddSlashes($url).'\'); return false;').'" value="'.GetMessage("admin_lib_filter_set_butt").'">
						<input type="submit" class="adm-btn" id="'.$this->id.'del_filter" name="del_filter" title="'.GetMessage("admin_lib_filter_clear_butt").$hkInst->GetTitle("del_filter").'" onClick="'.htmlspecialcharsbx($this->id.'.OnClear(\''.CUtil::AddSlashes($aParams["table_id"]).'\', \''.CUtil::AddSlashes($url).'\'); return false;').'" value="'.GetMessage("admin_lib_filter_clear_butt").'">';
	...
	public function End()
	{
		...
		$openedTab = $_SESSION["main.adminFilter"][$this->id]["openTab"];
		$filteredTab = $_SESSION["main.adminFilter"][$this->id]["filteredTab"];
...
//		BX.ready(function(){
			...
			if($filteredTab != null)
				echo '
			var filteredTab = "'.CUtil::JSEscape(htmlspecialcharsbx($filteredTab)).'";

			if(filteredTab)
				'.$this->id.'.FilteredTabMark(filteredTab);';

			echo '
			var openedTab = BX("adm-filter-tab-'.$this->id.'-'.CUtil::JSEscape(htmlspecialcharsbx($openedTab)).'");
...
class CAdminResult extends CDBResult
{
	...
	function GetNavPrint($title, $show_allways=true, $StyleText="", $template_path=false, $arDeleteParam=false)
	{
		...
		return parent::GetNavPrint($title, $show_allways, $StyleText, $template_path, array('action', 'sessid', 'LOAD_AUTH'));
	}
}
...
<?
		...
		if(
			...
			echo '<form method="POST" '.($this->bMultipart?' enctype="multipart/form-data" ':'').' onsubmit="return BX.ajax.submitComponentForm(this, \''.$this->table_id.'_result_div\', true);" id="form_'.$this->table_id.'" name="form_'.$this->table_id.'" action="'.htmlspecialcharsbx($APPLICATION->GetCurPageParam("mode=frame", array("mode"))).'">';
...
<?
		if($bShowSelectAll):
?>
			<td class="adm-list-table-cell adm-list-table-checkbox" onclick="this.firstChild.firstChild.click(); return BX.PreventDefault(event);"><div class="adm-list-table-cell-inner"><input class="adm-checkbox" type="checkbox" id="<?=$this->table_id?>_check_all" onclick="<?=$this->table_id?>.SelectAllRows(this); return BX.eventCancelBubble(event);" title="<?=GetMessage("admin_lib_list_check_all")?>" /></div></td>
<?
			$colSpan++;
		endif;
...
?>
	<span class="adm-selectall-wrap"><input type="checkbox" class="adm-checkbox" name="action_target" value="selected" id="action_target" onclick="if(this.checked && !confirm('<?=CUtil::JSEscape(GetMessage("admin_lib_list_edit_for_all_warn"))?>')) {this.checked=false;} <?=$this->table_id?>.EnableActions();" title="<?=GetMessage("admin_lib_list_edit_for_all")?>" /><label title="<?=GetMessage("admin_lib_list_edit_for_all")?>" for="action_target" class="adm-checkbox-label"><?=GetMessage("admin_lib_list_for_all");?></label></span>
<?
			endif;
...
?>
<script type="text/javascript">
window['<?=$tbl?>'] = new BX.adminList('<?=$tbl?>', {context_ctrl: <?=$aUserOpt["context_ctrl"] == "Y"? "true":"false";?>});
...
?>
	<td class="adm-list-table-cell adm-list-table-checkbox adm-list-table-checkbox-hover<?=$this->bReadOnly? ' adm-list-table-checkbox-disabled':''?>"><input type="checkbox" class="adm-checkbox" name="ID[]" value="<?=$this->id?>" autocomplete="off" title="<?=GetMessage("admin_lib_list_check")?>"<?=$this->bReadOnly? ' disabled="disabled"':''?><?=$this->bEditMode ? ' checked="checked" disabled="disabled"' : ''?> /></td>
<?
		endif;
...
<?
		...
		foreach($this->items as $n => $item)
		{
			...
			echo (!empty($item["LINK"])
				? '<a class="adm-navchain-item'.$className.'" href="'.$item["LINK"].'"'.(!empty($item["ONCLICK"])? ' onclick="'.$item["ONCLICK"].'"':'').'>'.$item["TEXT"].'</a>'
				: '<span class="adm-navchain-item'.$className.'">'.$item["TEXT"].'</span>'
			);
...
class CAdminCalendar
{
	...
	public function CalendarDate($sFieldName, $sValue="", $size="10", $bTime=false)
	{
		...
		return '
<div class="adm-calendar-block">
	<div class="adm-input-wrap">
		<input class="adm-input" type="text" name="'.$sFieldName.'" size="'.$size.'" value="'.htmlspecialcharsbx($sValue).'">
...
<div class="adm-calendar-block">';
	...
	$s .=''.
	'<div class="adm-input-wrap adm-calendar-inp adm-calendar-first" style="display: '.($bSelectShow ? 'none' : 'block').';">'.
		'<input type="text" class="adm-input adm-calendar-from" id="'.$sFromName.'_calendar_from" name="'.$sFromName.'" size="'.$size.'" value="'.htmlspecialcharsbx($sFromVal).'">'.
	...
	'</div>
	<span class="adm-calendar-separate" style="display:none"></span>'.
	'<div class="adm-input-wrap adm-calendar-second" style="display: '.($bSelectShow ? 'none' : 'block').';">'.
		'<input type="text" class="adm-input adm-calendar-to" id="'.$sToName.'_calendar_to" name="'.$sToName.'" size="'.$size.'" value="'.htmlspecialcharsbx($sToVal).'">'.
