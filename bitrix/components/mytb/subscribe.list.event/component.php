...
if (isset($_GET['clubID']) && intval($_GET['clubID']) != 0) {
    ...
    if (in_array($clubID, $rs['PROPERTY_LINK_EVENT_VALUE'])) {
        $arListClubSubs = array($clubID);
    }
} else {
    $arListClubSubs = $rs['PROPERTY_LINK_EVENT_VALUE'];
}
...
if (count($arListClubSubs) > 0) {
    $rsClub = Club::getList(array(), array("ID"=> $rs['PROPERTY_LINK_EVENT_VALUE']), FALSE, FALSE, array("NAME", "ID","PREVIEW_PICTURE"));
