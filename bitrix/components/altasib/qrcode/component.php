...
function fRGB($COLOR)
{
   ...
   for ($a=0; $a<6; $a++)
   {
    ...
    if($arr_tmp[$a] === false)return false;
...
if ($this->StartResultCache()) {
        ...
        if ($arParams["QR_TYPE_INF"] == "URL") {
                ...
                else $qrVal = "http://".$arParams["QR_URL"];
        }
        ...
        if ($arParams["QR_TYPE_INF"] == "VCARD") {
                $qrVal = "BEGIN:VCARD\n";
                if (strlen($arParams["QR_VC_FNAME"]) > 0) $qrVal = $qrVal."N:".$arParams["QR_VC_FNAME"].";";
                if (strlen($arParams["QR_VC_LNAME"]) > 0) $qrVal = $qrVal." ".$arParams["QR_VC_LNAME"]."\n";
                else $qrVal = $qrVal."\n";
        ...
        if ($this->__templateName == "input") {
                if (isset($_POST["QR_VALUE"]) && strlen($_POST["QR_VALUE"]) > 0 && strlen($_POST["GEN_QR"]) > 0) {
                        ...
                        if (BX_UTF !== true) $qrVal = $APPLICATION->ConvertCharset($_POST["QR_VALUE"], "windows-1251", "UTF-8");
                }
        } else {
        ...
        if (file_exists($_SERVER["DOCUMENT_ROOT"].'/upload/altasib/qrcode/'.$md.'.png')) $arResult["RESULT"] = "Y";
        else $arResult["RESULT"] = "N";
