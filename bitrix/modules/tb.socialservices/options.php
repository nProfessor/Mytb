...
$oAuthManager = new TrCSocServAuthManager();
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["Update"].$_POST["Apply"].$_POST["RestoreDefaults"] <> '' && check_bitrix_sessid())
{
	...
	{
		COption::SetOptionString("tr.socialservices", "use_on_sites", serialize($_POST["use_on_sites"]));
		foreach($arSiteList as $site)
		{
			$suffix = ($site <> ''? '_bx_site_'.$site:'');
			COption::SetOptionString("tr.socialservices", "tr_auth_services".$suffix, serialize($_POST["TR_AUTH_SERVICES".$suffix]));
...
?>
				<tr>
					<td>
						<input type="hidden" name="TR_AUTH_SERVICES<?=$suffix?>[<?=htmlspecialchars($id)?>]" value="N">
						<input type="checkbox" name="TR_AUTH_SERVICES<?=$suffix?>[<?=htmlspecialchars($id)?>]"
							id="TR_AUTH_SERVICES<?=$suffix?><?=htmlspecialchars($id)?>"
							value="Y"
					...
					<td><label for="TR_AUTH_SERVICES<?=$suffix?><?=htmlspecialchars($id)?>"><?=htmlspecialchars($service["NAME"])?></label></td>
					<td>&nbsp;</td>
					<td><a href="javascript:void(0)" onclick="MoveRowUp(this)"><img src="/bitrix/images/tr.socialservices/up.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_up")?>" border="0"></a></td>
					<td><a href="javascript:void(0)" onclick="MoveRowDown(this)"><img src="/bitrix/images/tr.socialservices/down.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_down")?>" border="0"></a></td>
				</tr>
<?
	endforeach;
?>
